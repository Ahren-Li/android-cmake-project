...
function(getMoudleExport module_name type out)
    SET(${out} "" PARENT_SCOPE)
    if(EXISTS "${MK_EXP_DIR}/${module_name}_${type}.export")
        file(STRINGS "${MK_EXP_DIR}/${module_name}_${type}.export" getMoudleExport_list)
        SET(${out} "${getMoudleExport_list}" PARENT_SCOPE)
    endif()
endfunction()
...
function(doNeedExport module_type list)
    if(EXISTS "${MK_NEED_EXP_DIR}/${module_type}.list")
        ...
        foreach(doNeedExport_moudle ${doNeedExport_list})
            string(REPLACE ":" "_" doNeedExport_moudle "${doNeedExport_moudle}")
            foreach(doNeedExport_dir ${list})
                if(LOAD_MOUDLE_DEBUG)
                    message("doNeedExport ${doNeedExport_moudle} add include ${doNeedExport_dir}")
                endif()
                saveExportInclude("${doNeedExport_moudle}" "${doNeedExport_dir}")
            endforeach()
        endforeach()
    endif()
endfunction()
